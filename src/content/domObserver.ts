...
class DOMObserverImpl {
  ...
  private checkForCommentInputFields(element: HTMLElement): void {
    ...
    // Check descendants
    const inputFields = element.querySelectorAll(
  ...
  private checkForSubmittedComments(element: HTMLElement): void {
    ...
    // Deduplicate elements within this mutation batch
    const seen = new Set<HTMLElement>();

    const tryCapture = (el: HTMLElement): void => {
      if (seen.has(el)) return;
      seen.add(el);
      this.captureCommentSubmission(el);
    };
    ...
    commentSelectors.forEach((selector) => {
      ...
      element.querySelectorAll(selector).forEach((commentEl) => {
        tryCapture(commentEl as HTMLElement);
      });
    });
  }
  ...
  private isCommentInputField(element: HTMLElement): boolean {
    ...
    const textToCheck = [role, ariaLabel, placeholder]
      .filter(Boolean)
      .join(" ");
  ...
  private getTextContent(element: HTMLElement): string {
    ...
    const lexicalContent = element.querySelector(
  ...
  private extractCommentId(element: HTMLElement): string {
    // Check our own stamp first — prevents a re-detected element from
    // generating a new fallback ID and bypassing processedCommentIds.
    const stamped = element.getAttribute("data-boongai-comment-id");
  ...
  private extractPostId(element: HTMLElement): string {
    ...
    while (current && current !== document.body) {
      ...
      const postIdAttributes = ["data-post-id", "data-postid", "data-ft"];
